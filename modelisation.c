...
int main(int argc, char *argv[])
{
	...
	taille_cycle = atoi(argv[2]);
	distance_LC = atoi(argv[3]);
	nb_arete_base=0;
	taille_base=0;

	chebi_id = mol_courante;
	init_atom_num ();
	...
	char src[64];
	sprintf(src,"../resultats/similarite_LC_%d_%d_%d_all.data",mol_courante,taille_cycle,distance_LC);
	...
	char distrib[64];
	sprintf(distrib,"../resultats/distri_LC_%d_%d_%d_all.data",mol_courante,taille_cycle,distance_LC);
	//fichiers des resultats 
	...
	char temps[64];
	sprintf(temps,"../resultats/temps_LC_test_%d_%d_%d.data",mol_courante,taille_cycle,distance_LC);
